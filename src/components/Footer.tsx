    ...
    <footer className={`w-full border-t ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'} pt-12 pb-6`}>
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="flex flex-col items-center md:items-start space-y-2">
          ...
          <span className={`text-sm ${isDarkMode ? 'text-purple-200' : 'text-purple-700'}`}>Created with <span className="text-red-400">♥</span> by Orincore</span>
          <span className={`text-xs mt-2 ${isDarkMode ? 'text-purple-300' : 'text-purple-800'}`}>© {new Date().getFullYear()} Orincore. All rights reserved.</span>
        </div>
        {/* Quick Links */}
        ...
        <div className="flex flex-col items-center md:items-end space-y-2">
          ...
          <span className={`text-xs ${isDarkMode ? 'text-purple-200' : 'text-purple-700'}`}>contact@orincore.com</span>
        </div>
      </div>
    </footer>
  );
