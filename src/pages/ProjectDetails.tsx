...
const ProjectDetails: React.FC = () => {
  ...
  return (
    <>
      <Helmet>
        <title>{project.title ? `${project.title} | Orincore Project Details` : 'Project Details | Orincore'}</title>
        <meta name="description" content={project.description ? project.description : 'See project details, features, and tech stack for this Orincore portfolio project.'} />
        <meta name="keywords" content={project.tech_stack ? (Array.isArray(project.tech_stack) ? project.tech_stack.join(', ') : project.tech_stack) : 'Orincore, project, portfolio, web development, mobile, AI, IoT'} />
        <meta property="og:title" content={project.title ? `${project.title} | Orincore Project Details` : 'Project Details | Orincore'} />
        <meta property="og:description" content={project.description ? project.description : 'See project details, features, and tech stack for this Orincore portfolio project.'} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://orincore.com/portfolio/${project.id || ''}`} />
        <meta property="og:image" content={images && images.length > 0 ? images[0] : 'https://orincore.com/assets/logo/OClogo.png'} />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={project.title ? `${project.title} | Orincore Project Details` : 'Project Details | Orincore'} />
        <meta name="twitter:description" content={project.description ? project.description : 'See project details, features, and tech stack for this Orincore portfolio project.'} />
        <meta name="twitter:image" content={images && images.length > 0 ? images[0] : 'https://orincore.com/assets/logo/OClogo.png'} />
        <link rel="canonical" href={`https://orincore.com/portfolio/${project.id || ''}`} />
        <script type="application/ld+json">{`
          {
            "@context": "https://schema.org",
            "@type": "CreativeWork",
            "name": "${project.title ? project.title.replace(/"/g, '') : 'Orincore Project'}",
            "description": "${project.description ? project.description.replace(/"/g, '') : 'See project details, features, and tech stack for this Orincore portfolio project.'}",
            "url": "https://orincore.com/portfolio/${project.id || ''}",
            "image": "${images && images.length > 0 ? images[0] : 'https://orincore.com/assets/logo/OClogo.png'}",
            "author": {
              "@type": "Organization",
